    ...
    while running:
        ...
        for event in pygame.event.get():
            if event.type == QUIT or (event.type == KEYDOWN and event.key == K_ESCAPE):
                ...
                if np.linalg.norm(cam_pos - np.array([0.0, 0.0, -5.0])) < 4.0 and current_room == 0:
                    if dialogo_ativo:
                        dialogo_ativo, char_index_di, current_di = False, 0, 0
                    else:
                        dialogo_ativo, char_index_di, current_di, texto_atual_exibido = True, 0, 0, ""
                elif np.linalg.norm(cam_pos - np.array([0.0, 0.0, -25.0])) < 4.0 and current_room == 0:
                    ...
                    for ini in inimigos:
                        if ini['alive']:
                            ...
                            if np.dot(cam_front, to_enemy) > 0.985 and np.linalg.norm(ini['position'] - cam_pos) < 25.0: 
                                ...
                                if num_inimigos_vivos == 0 and som_morte: som_morte.play()
                                break 
